...
from utils.auth import initialize_auth_state, show_auth_page, show_user_info, check_authentication, show_woocommerce_orders
...
if not check_authentication():
    ...
    with col2:
        if user_id:
            ...
            st.metric("API Queries Used", "0/30")
    ...
    with col3:
        if user_id:
            ...
            st.metric("Remaining Queries", "30")
    ...
    if user_id:
        ...
        if orders_summary:
            ...
            with col1:
                st.metric("Total Orders", orders_summary['total_orders'])
            
            with col2:
                st.metric("Total Spent", f"${orders_summary['total_spent']:.2f}")
            
            with col3:
                st.metric("Completed Orders", orders_summary['completed_orders'])
            
            with col4:
                avg_order = orders_summary['total_spent'] / orders_summary['total_orders'] if orders_summary['total_orders'] > 0 else 0
                st.metric("Average Order", f"${avg_order:.2f}")
            
            # Show recent orders
            if orders_summary['recent_orders']:
                st.subheader("📋 Recent Orders")
                for order in orders_summary['recent_orders'][:3]:
                    with st.expander(f"Order #{order.get('wc_order_id')} - ${order.get('total', 0):.2f}"):
                        col1, col2 = st.columns(2)
                        with col1:
                            st.write(f"**Status:** {order.get('status', 'N/A').title()}")
                            st.write(f"**Date:** {order.get('date_created', 'N/A')[:10]}")
                        with col2:
    ...
    st.markdown("---")
    st.info("💡 **Navigation:** Use the sidebar to access different features of the application. Your WordPress login session is maintained across all pages.")
    
    # Footer
    st.markdown("---")
    st.caption("🔒 **Security:** This application uses WordPress JWT authentication with Supabase data synchronization. Your data is encrypted and securely stored.")
